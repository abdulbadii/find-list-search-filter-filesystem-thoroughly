...
[ "$D$dtx" ]&&echo "${D+Option \"$Dt\" is depth '$D'${Rd+ reversed from max $DM} of $M}${D+${dtx+ and }}${dtx+$dtx of $M}">&2
	...
	if((de))&&[[ $z != / ]];then	export -f fid;find ${C[@]} ! -type d -executable -exec bash -c 'fid \"\$0\" \"\$@\"' '{}' \;
		...
		#exec 3>&1 4>&2;E=$(2> >(while read s;do echo -e "\e[1;31m$s\e[m">&4;done) sudo find $L ${C[@]} $EO |tee >(read -rN1;echo $?)>&3)
		E=`sudo find $L ${C[@]} $EO |read -rn1;echo $?`
		2> >(while read s;do echo -e "\e[1;31m$s\e[m">&4;done) sudo find $L ${C[@]} $EO
		((OL+N0+RM))&&{
