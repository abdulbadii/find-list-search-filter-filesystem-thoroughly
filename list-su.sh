...
if((de));then	export -f fid;find "${A[@]}" "${PS[@]}" "${B[@]}" \) ! -type d -executable -exec bash -c 'fid "$0" "$@"' '{}' \;
		...
		echo -e "\e[41;1;33m${m%[!/]}\e[m">&2;done)	sudo find "${A[@]}" "${PS[@]}" "${B[@]}" \) "${E[@]}"
else
((OL+EM)) || sudo find "${A[@]}" ${CT+-type d} "${PS[@]}" "${B[@]}" \) "${E[@]}"&&((LN))&&{ echo Link resolution:;sudo find "${A[@]}" "${B[@]}" \) "${PL[@]}";}>&2
...
((RF+CM))&&{
	...
	((RF))&&{ E=
		...
		a=remov;}
	((C))&&a=copi;((MV))&&a=mov
	sudo find "${A[@]}" -print "${B[@]}" \)| read -n1||{ echo Nothing has been found and ${a}ed>&2;return;}
	if((RF));then
		((OL+EM))&&sudo find $L "${A[@]}" "${PS[@]}" "${B[@]}" \) "${E[@]}"
		...
		(if((CT));then
			sudo find "${S[@]}" ${opt[@]} -type d "${A[@]}" "${B[@]}" \) -exec bash -c "for d ;{ mkdir -p $c/\$d;chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d;} " '{}' +
		elif [ ! $CR ];then
			sudo find $po "${S[@]}" ${opt[@]} "${A[@]}" "${PS[@]}" "${B[@]}" \) |sudo xargs bash -c "for d ;{ if [ -d \"\$d\" ];then mkdir -p $c/\$d; chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d; else cp $CO $CP \"\$d\" $c;fi;}"
		else
			sudo find $po "${S[@]}" ${opt[@]} $ND "${A[@]}" "${PS[@]}" "${B[@]}" \) |sudo xargs -i cp $CO $CP $CR '{}' "$c"
		fi
