...
[ "$po" = -L ] ||{
	sudo find . -type l -xtype d |tee /tmp/.0 |read -n1 &&{
		mapfile -t a</tmp/.0
		echo 'Search into which the below link(s) point(s) ?'>&2;sudo find "${a[@]}" "${PL[@]}"
		read -N1 -p '(Enter: Yes)' i>&2
		[ "$i" = $'\x0a' ] &&{
			sudo find -L "${a[@]}" ${opt[@]} -regextype posix-extended \( -${I}regex "$R" "${X[@]}" \( "${PD[@]}" -o "${PT[@]}" \) \)
	};}
}
...
((RF+CM))&&{
	...
	((RF))&&{ E=
		...
		a=remov;}
	((C))&&a=copi;((MV))&&a=mov
	sudo find $L "${A[@]}" "${AA[@]}" \) "${E[@]}"| read -n1||{ echo Nothing has been found and ${a}ed>&2;return;}
	if((RF));then
		((OL+EM))&&sudo find $L "${A[@]}" "${AA[@]}" \) "${E[@]}"&&((OL))&&{ echo Orphan link resolution:;sudo find "${A[@]}" "${AA[@]}" \) "${PL[@]}";}>&2
		eval read -N1 -p \"Delete$PO o>&2;[ "$o" = $'\x0a' ]||return
		sudo find $L "${A[@]}" "${PP[@]}" "${AA[@]}" \) "${E[@]}" -delete &&echo All deleted>&2
	elif((C));then
		...
		(if((CT));then
			sudo find "${S[@]}" ${opt[@]} -type d "${A[@]}" "${AA[@]}" \) -exec bash -c "for d ;{ mkdir -p $c/\$d;chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d;} " '{}' +
		elif [ ! $CR ];then
			sudo find $po "${S[@]}" ${opt[@]} "${A[@]}" "${PS[@]}" "${AA[@]}" \) |sudo xargs bash -c "for d ;{ if [ -d \"\$d\" ];then mkdir -p $c/\$d; chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d; else cp $CO $CP \"\$d\" $c;fi;}"
		else
			sudo find $po "${S[@]}" ${opt[@]} $ND "${A[@]}" "${PS[@]}" "${AA[@]}" \) |sudo xargs -i cp $CO $CP $CR '{}' "$c"
		fi
