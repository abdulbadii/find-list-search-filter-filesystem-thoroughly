...
((RF+CM))&&{	unset o E ND
	...
	if((RF));then
		...
		if((CT));then
			...
			(sudo find $po "${S[@]}" ${opt[@]} "${A[@]}" "${PS[@]}" "${AF[@]}" \) |sudo xargs bash -c "for d ;{ if [ -d \"\$d\" ];then mkdir -p $c/\$d; chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d; else cp $CO $CP \"\$d\" $c;fi;}" &&echo Successfuly copied)2> >(while read s;do echo -e "\e[1;31m$s\e[m">&2;done)
		else
