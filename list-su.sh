...
if((de+if)) &&[ $F$LN ] ;then
...
#elif [ $cpu ] ;then
	#sudo mkdir -pv $cpe
	#eval "sudo find $po $s $d \! -ipath $s $P $opt $XC -exec cp -ft '{}' $cpe \;"
	...
	command 2> >(while read s;do echo -e "\e[1;31m$s\e[m" >&2; done) eval "sudo find $po $s -regextype posix-extended $d \! -ipath $s -${I}path $P $opt $XC $Z"
fi
}
}
}
