...
if((Fc));then
	if [ -d $C ];then read -n1 -p "wipe \"$C\" out and replace with the search result?" o;[ $o = y]&&sudo rm -r $C
	elif	[ -f $C ];then echo Trying to replace file $C>&2;rm $vb $C||{ echo Failed, try again as root>&2;sudo rm $vb $C;}
	fi;U=${U#\|};V=${V# -o }
	eval "sudo find $po$T -regextype posix-extended $opt\( $Rt ${U:+-${I}regex \"$U\"} ${V:+$V} \) ${X[@]-$Z} -exec cp -ru$vb '{}' \"$C\" \;"
#elif((Fz));then	eval set -- $C;for i;{
	#[ -d $i ]||{
		#[ -f $i ]&&{ echo Trying to replace file $i>&2;rm $vb $i||{ echo Failed, try again as root>&2;sudo rm $vb $i;};}
		#i=$cp$i
		#mkdir -p$vb $i||{ echo Failed, try again as root>&2;sudo mkdir -p$vb $i;};}
