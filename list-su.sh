...
((OL+EM)) ||sudo find "${A[@]}" ${CT+-type d} "${PS[@]}" "${AF[@]}" \) "${E[@]}"&&((LN))&&{ echo Link resolution:;sudo find "${A[@]}" "${AF[@]}" \) -type l \( -path '* *' -printf "'%p' -> '%l'\n" -o -printf "%p -> %l\n" \);}>&2
...
((RF+CM))&&{	unset o E ND
	...
	if((RF));then
		...
		if [ -e $c ];then
			...
			read -N1 -p "Or if no instead, copy the above merge into it or abort? (n: no, else: abort) " o>&2
		...
		if((C1));then CP=;CR=;ND='! -type d'
			...
			S=$ch
			PS+=([-9]=-printf "'%P'\n");for i in {-7..-1} ;{ unset PS[i];}
		fi

		if((F));then
			A=(\( -${I}path "$Q/*" "${X[@]}");AF=(${p:+-o \( -${I}path "$Q" -o -${I}path "$S/*$p" \) "${PP[@]}"})
		else			A=(-regextype posix-extended \( -${I}regex "$R" "${X[@]}");fi

		(if((CT));then	sudo find "${S[@]}" ${opt[@]} -type d "${A[@]}" "${AF[@]}" \) -exec bash -c "for d ;{ mkdir -p $c/\$d;chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d;} " '{}' +
		elif [ ! $CR ];then
			sudo find $po "${S[@]}" ${opt[@]} $ND "${A[@]}" "${PS[@]}" "${AF[@]}" \) |sudo xargs bash -c "for d ;{ if [ -d \"\$d\" ];then mkdir -p $c/\$d; chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d; else cp $CO $CP \"\$d\" $c;fi;}"
		else
			sudo find $po "${S[@]}" ${opt[@]} "${A[@]}" "${PS[@]}" "${AF[@]}" \) |sudo xargs -i cp $CO $CP $CR $S/'{}' "$c"
		fi
		(($?))||echo -n Successfully copied)
		#2> >(while read s;do echo -e "\e[1;31m$s\e[m">&2;done)
		}
	elif((MV));then
		:
	fi
