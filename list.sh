...
if((de));then	export -f fid;find "${A[@]}" "${P[@]}" "${B[@]}" \) ! -type d -executable -exec bash -c 'fid "$0" "$@"' '{}' \;
		...
		echo -e "\e[41;1;33m${m%[!/]}\e[m">&2;done)	find $L "${A[@]}" "${P[@]}" "${B[@]}" \) "${E[@]}"
else
	((RF))&&{
		((RM))&&2> >(while read s;do echo -e "\e[1;31m$s\e[m">&2;done) find "${A[@]}" "${P[@]}" "${B[@]}" \) "${E[@]}"
		((EM))&&	E=(-empty "${PE[@]}")
	...
	((RM))||2> >(while read s;do echo -e "\e[1;31m$s\e[m">&2;done) find $L "${A[@]}" "${P[@]}" "${B[@]}" \) "${E[@]}"
	((PL))&&{ echo;echo "Link resolution (shown on non-standard form to stderr, to keep it from piping):";find $L "${A[@]}" "${B[@]}" \) -type l \( -path '* *' -printf "'%p' -> '%l'\n" -o -printf "%p -> %l\n" \)>&2;}
	if((RF));then
		find $L "${A[@]}" "${B[@]}" \) "${E[@]}"| read -rn1 ||{ echo Nothing was found and deleted>&2;return;}
		...
		find "${A[@]}" "${B[@]}" \)|read -rn1 ||{ echo Nothing was found and copied>&2;return;}
		...
		if((CH));then
			[ "$ch" ]&&{
				...
				[ $po ];A[1-$?]=$ch
				echo find "${A[@]}" \( -path '* *' -printf "'%P'$sz$tm\n" -o -printf "%P$sz$tm\n" \) "${B[@]}" \) "${E[@]}" |xargs -i cp --parents -ur '{}' $c

				return
			}
		else
			echo find "${A[@]}" "${P[@]}" "${B[@]}" \)|xargs -i cp --parents -ur '{}' $c
