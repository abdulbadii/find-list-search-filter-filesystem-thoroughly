...
a=$B${a%%+(/)}
[ -z $a ] &&{
	((re))||d="-maxdepth 1"; eval "find $po ~+ $d \! -ipath ~+ $opt $XC $Z" ;continue;}
unset p n P Z
if [[ $a =~ ^/ ]] ;then re=
	while [[ $a =~ /([^.].|.[^.]|[^/]{3,}|[^/])/\.\.(/|$) ]];do a=${a/"${BASH_REMATCH[0]}"/\/};done
		[[ $a =~ ^/..(/|$) ]] &&{ echo Invalid actual path: $a >&2;continue;}
...
#elif [ $cp ] ;then
	#mkdir -pv $cp
	#eval "find $po $s $d \! -ipath $s $P $opt $XC -exec mkdir -p ${cp[0]}/\{\} &>/dev/null \; -exec cp -r '{}' ${cp[0]}/\{\} \;"
#elif [ $cpe ] ;then
	#mkdir -pv $cpe
	#eval "find $po $s $d \! -ipath $s $P $opt $XC -exec cp -r '{}' $cpe \;"
	
else
	command 2> >(while read s;do echo -e "\e[1;31m$s\e[m" >&2; done) eval "find $po $s $d \! -ipath $s $P $opt $XC $Z"
fi
}
}
}
