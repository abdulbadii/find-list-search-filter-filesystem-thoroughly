...
((OL+EM)) || find "${A[@]}" ${CT+-type d} "${PS[@]}" "${AF[@]}" \) "${E[@]}"&&((LN))&&{ echo Link resolution:;sudo find "${A[@]}" "${AF[@]}" \) "${PL[@]}";}>&2
...
((RF+CM))&&{	unset o E ND
	...
	((C))&&a=copi;((MV))&&a=mov
	find "${A[@]}" -print "${AF[@]}" \)| read -n1||{ echo Nothing has been found and ${a}ed>&2;return;}
	if((RF));then
		...
		(if((CT));then
			find "${S[@]}" ${opt[@]} -type d "${A[@]}" "${AF[@]}" \) -exec bash -c "for d ;{ mkdir -p $c/\$d;chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d;} " '{}' +
		elif [ ! $CR ];then
			find $po "${S[@]}" ${opt[@]} "${A[@]}" "${PS[@]}" "${AF[@]}" \) |sudo xargs bash -c "for d ;{ if [ -d \"\$d\" ];then mkdir -p $c/\$d; chmod --reference=\$d $c/\$d; chown --reference=\$d $c/\$d; touch --reference=\$d $c/\$d; else cp $CO $CP \"\$d\" $c;fi;}"
		else
			find $po "${S[@]}" ${opt[@]} $ND "${A[@]}" "${PS[@]}" "${AF[@]}" \) |sudo xargs -i cp $CO $CP $CR '{}' "$c"
		fi
